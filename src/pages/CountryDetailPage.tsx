...
import {
  ...
  disasterZones, countryDefaultCenters, type CountryStatus
} from '@/data/mockData';
import { ArrowLeft, AlertTriangle, Users, MapPin, ChevronRight, Activity } from 'lucide-react';
    ...
    <div className="h-full overflow-y-auto bg-background pb-6">
      ...
      {status.prediction && (
        <div className="mx-4 mt-4 rounded-xl p-3 border border-border"
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold text-foreground">Risk Forecast</span>
          </div>
      ...
      {status.recentEvents.length > 0 && (
        <section className="px-4 mt-5">
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Recent Events</h2>
      ...
      {countryZones.length > 0 && (
        <section className="px-4 mt-5">
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Disaster Map</h2>
      ...
      {countryNews.length > 0 && (
        <section className="px-4 mt-5">
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Related News</h2>
          <div className="space-y-2">
            {countryNews.map(news => (
              <div key={news.id} className="flex gap-3 rounded-xl bg-card border border-border p-2">
                <img
